...
class isf(object):
    ...
    def __init__(self, path, debug=False, buffer_size=4096):
        ...
        while True:
            ...
            hdr_start = buffered_search(f, b':WFMP:', start=f_pos, buffer_size=buffer_size)
            if hdr_start == -1:
                hdr_start = buffered_search(f, b':WFMPRE:', start=f_pos, buffer_size=buffer_size)
                if hdr_start == -1:
                    if debug > 0:
                        debug_str += 'Neither header descriptors (":WFMP:" or ":WFMPRE:") are found.'
                        print(debug_str)
            ...
            data_start = buffered_search(f, b':CURV #', start=hdr_start, buffer_size=buffer_size)
            if data_start == -1:
                data_start = buffered_search(f, b':CURVE #', start=hdr_start, buffer_size=buffer_size)
