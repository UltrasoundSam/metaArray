...
class flex_files(object):
    ...
    def parse_headers(self, path = None, debug = False):
        """
        ...
        self.rate = False # Sampling rate, only meaningful if flg_resample == True.
        self.flg_resample_immune = False # Immune to resampling, internal use only
        
        self.records = None     # An instant of Fortran binary record file.
        self.endian = None 
        
        self.hdr1 = None        # Header record structures
        self.hdr2 = None
        self.hdr3 = None
        self.hdr4 = None
        self.hdr_nfo = None     # This is the internal storage of parsed record info 
        ...
        # print length, int_type, calcsize(int_type)
        #for i in ['i', 'l', 'h', 'q']:
        #    if length == calcsize(i):
        ...
        try:
            ...
            raise ValueError, "The first record do not appear to have the right length. \
        ...
        if idata == None:
            # Bad luck, cant match the byte length.
            raise ValueError, "The byte length (" + str(int_byte) + ") of int variable \
            specified in header record 4 can not be matched to any \
            available int data types."
        
        if fdata == None:
            # Bad luck, cant match the byte length.
            raise ValueError, "The byte length (" + str(float_byte) + ") of float \
    ...
    def get_data_item(self, key):
        ...
        if key > nrecd:
            raise ValueError, 'Requested Fortrtan record number (' + str(key) + \
            ') does not exist, maximum availabe record number is: ' + \
            str(nrcd - 1)
        elif key < 0:
            raise ValueError, 'Requested Fortran record number (' + str(key - nrecd) + \
            ') does not exist.'
    ...
    def showIndex(self, N = None):
        ...
        """
        return self.records.showIndex(N = None)
    
    def parse(self, structure, record, strip = False):
        """
        ...
        Exceptions:
            If name is 'data', then use numpy to unpack directly
                ...
                reversedand and the data array will be transposed by 
                default. (i.e. C shape input -> read in Fortran order -> return data in C standard)
        ...
        # for i in structure:
            unpack_str = endian + str(v_length) + v_type # generate the unpack string
            length = calcsize(str(v_length) + v_type) # Length of the current entry
            ...
            ############################################################
            #
            # Aviod the use of struct.unpack because of padding issues:
            # 
            # >>> pack('<1l', 2)
            # '\x02\x00\x00\x00'
            # >>> pack('1l', 2)
            ...
            # 
            # >>> unpack('1l', '\x02\x00\x00\x00\x00\x00\x00\x00')
            # (2,)
            # >>> unpack('<1l', '\x02\x00\x00\x00\x00\x00\x00\x00')
            ...
            #        current_entry = current_entry.reshape(v_val[::-1]).transpose()
            #else:
            #    # Unpack the byte stream as numbers
            #    current_entry = unpack(unpack_str, record[current_index:next_index])
            #    if v_length == 1:
            #        # Unpack will always return a list.
            #        # If there is only one element in the list anyway, keep only the element.
...
class data_out1(flex_files):
    ...
    def __init__(self, path = None, debug = False):
        ...
        self.desc_lst = None        # The list of data record descriptions (index)
        ...
        idata = self.idata 
        fdata = self.fdata 
        hdr_int_byte = self.hdr_int_byte
        hdr_idata = self.hdr_idata
        ...
        # B(2): (((data(i,j,k), i=ibegin, iend), j=jbegin, jend), k=kbegin, kend)
        # B2 = []
    ...
    def _index_A_(self):
        ...
        if debug: print("Indexing the file for A records... ")
        ...
        while i < (nrcd + 3):
            ...
            if ndim == 3:
                ...
                B[3] = (jrange, irange)
                if krange != 1:
                    raise ValueError, "The record header (A.ndim) suggests this is a \
                    two dimentional array, conflicts with A.krange, A.krange should be 1.\
                ...
                raise ValueError, "The record header (A.ndim = " + str(ndim) + ") is incorrect. \
                Current Fortran record index is: " + str(i)
            ...
            if ipart == 1:
                ...
                B_pos = i + 1
                B_len = B[2]            # Difficult to do byte count, could be double precision
                while B_len > 0:
                ...
                if B_len != 0:
                    ...
                    raise ValueError, "Residual B record length incorrect, the " + linesep + \
                                    "total length of B records parts should be " + linesep + \
                                    "the same as the product of array ranges."  + linesep + \
                                    "Current Fortran record number: " + str(i) + linesep + \
                                    "irange: " + str(irange) + linesep + \
                                    "jrange: " + str(jrange) + linesep + \
                                    "krange: " + str(krange) + linesep + \
                                    "Residual B record length: " + str(B_len)
            else:
                raise ValueError, "ipart value in A record can only be 1 or 0, " + linesep + \
                                "got " + str(ipart) + "instead, current Fortran " + linesep + \
                                "record number is: " + str(i)
            ...
            if itype == 1:
                ...
                raise ValueError, "itype value in A record can only be from -20 to 1, \
                                got " + str(itype) + "instead, current Fortran \
                                record number is: " + str(i)
        ...
        self.nentry = len(A_idx)              # Number of data entries
        self.flg_ent_indexed = True
        self.A_idx = A_idx      
        return
        
    
    def transpose(self, name, range):
        """
    ...
    def showIndex(self):
        ...
        for i in range(self.nentry):
            desc += str(i) + '\t' + \
                    ...
                    '%(#)0.3e' % {"#": A_idx[i][2]} + '\t' + \
                    str(A_idx[i][3][3]) + '\t' + \
                    str(A_idx[i][6]) + '->' + str(A_idx[i][7]) + linesep
        return desc
    
    def resampled(self, key, smp_rate = False):
        """
    ...
    def global_resample(self, switch, smp_rate = False):
        """
    ...
    def __repr__(self):
        ...
        desc = 'This is a PZFlex data_out1 file object at: ' + self.fpath.full + linesep
        desc += '\tNumber of data records: ' + str(self.nentry) + linesep
        desc += 'Index\tName\tntmstp\tTime\t\tShape\t\tRange' + linesep
        desc += self.showIndex()
        
        if self.flg_resample == True:
            desc += linesep
            desc += '=' * 72 + linesep
            
            if self.rate == False:
                desc += 'Items returned will be automatically resampled to the next ' + linesep
                desc += 'highest standard sampling rate according to metaArray.' + linesep
            else:
                desc += 'Items returned will be automatically resampled at: ' + str(self.rate) + ' samples per unit time.' + linesep
...
class pout_hist(flex_files):
    ...
    def __init__(self, path = None, debug = False):
        ...
        self.flg_resample = False
        self.rate = False           # (re)Sampling rate, only meaningful if flg_resample == True.
        # self.flg_resample_immune = False # Immune to resampling, internal use only
        self.t0 = False             # Start time according to the time record
        self.t1 = False             # End time according to the time record
        self.tpts = False           # Number of (time) samples between self.t0 and self.t1
        ...
        self.nrecd = None           # Number of data records on the file including the time record
        self.ntim = None            # Number of time values for which time history data was saved
        self.ngrd = None            # Number of 20-character words for grid name = 1
        self.ndes = None            # Number of 20-character words for description = 1
        self.ntag = None            # Number of 20-character words for curve tag = 1
        self.nshift = None          # 1 = time shift for each record; 0 = no time shift
        self.nlabl = None           # Number of 20-character words for curve lable = 1
        self.nindx = None           # Number of indicies for each record
        self.ncord = None           # Number of coordinates for each record
        self.nchr = None            # Number of bytes per character word = 20
        ...
        if base_desc['nshift'] == 0:
            ...
            raise ValueError, 'Unexpected time shift value in the base record of the file.' + \
            'Expecting "1" or "0", but got ' + str(base_desc['nshift']) + ' instead.'
        ...
        self.t0 = tary.data[0]              # Start time according to the time record
        self.t1 = tary.data[-1]             # End time according to the time record
        self.tpts = len(tary)               # Number of (time) samples between self.t0 and self.t1
    ...
    def __repr__(self):
        ...
        desc = 'There are ' + str(len(self)) + ' data record(s) in this PZFlex POUT HIST file object at: ' + linesep
        desc += '\t' + self.fpath.full + linesep
        desc += '=' * 72 + linesep
        desc += self.showIndex()
        desc += '=' * 72 + linesep
        desc += 'The time history records started at time: ' + str(self.t0) + linesep
        desc += 'The time history records finished at time: ' + str(self.t1) + linesep
        desc += 'There are ' + str(self.tpts) + ' time samples, with the equivalent sampling rate of: ' + str(self.tpts / (self.t1 - self.t0)) + linesep
    ...
    def showIndex(self):
        ...
        desc += 'This flex pout hist file is generated at: ' + self.date + linesep
